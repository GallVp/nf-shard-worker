...
func (r *queryResolver) HealthCheck(ctx context.Context) (bool, error) {
	r.Logger.Info("Received a health check request", "response", "true")
	return true, nil
}
...
func (r *queryResolver) CheckStatus(ctx context.Context, executor model.Executor) (*model.CheckStatusResponse, error) {
	...
	case "float":
		...
		return &model.CheckStatusResponse{Status: status, Message: msg}, nil
	default:
		return &model.CheckStatusResponse{Status: false, Message: "could not find executor"}, errors.New("could not find executor")
	}
}
